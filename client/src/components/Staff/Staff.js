...
const Staff = () => {
  ...
  const handleLogin = async(event) => {
    ...
    try {
      const result = await axios.post("http://localhost:8000/api/staff/login", {
        email,
        password
      })
      ...
      window.localStorage.setItem("staffId" , result.data.data.staffId);
  ...
  const handleRegister = async(event) => {
    ...
    try {
      const result = await axios.post("http://localhost:8000/api/staff/register", {
        name,
        email,
        password
      })
